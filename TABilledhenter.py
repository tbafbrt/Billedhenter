...
class ICRTImageDownloader:
    ...
    def search_images_for_codes(self, project_code: str, webkodes: List[str]) -> Dict:
        ...
        # Create normalized lookup sets for better matching
        # Normalize webkodes for matching (remove optional letter prefixes)
        normalized_webkodes = {}  # Maps normalized code to original webkode
        for webkode in webkodes:
            clean_webkode = webkode.strip()
            normalized_code = normalize_code_for_matching(clean_webkode)
            normalized_webkodes[normalized_code] = clean_webkode
        ...
        # Enhanced extract_product_code function to handle optional letter prefixes
        def extract_product_code(filename):
            """Extract product code from filename, handling optional letter prefixes"""
            # First extract the base part (before underscore or parentheses)
            if '_' in filename:
                base_part = filename.split('_')[0].strip()
            elif '(' in filename:
                base_part = filename.split('(')[0].strip()
            else:
                base_part = filename.strip()
            
            return base_part.lower()
        
        def normalize_code_for_matching(code):
            """Normalize a code for matching by removing optional letter prefix"""
            code = code.strip().lower()
            
            # Check if code starts with letters followed by numbers (e.g., "ic23022-0254-50")
            # Pattern: 1-3 letters + numbers-numbers-numbers
            match = re.match(r'^([a-z]{1,3})(\d+[-]\d+[-]\d+.*)
        ...
        for i, media in enumerate(media_files):
            ...
            if filename and image_url:
                ...
                file_product_code = extract_product_code(filename)
                
                # Try multiple matching strategies
                matched_webkode = None
                
                # Strategy 1: Direct match (exact as extracted)
                if file_product_code in normalized_webkodes:
                    matched_webkode = normalized_webkodes[file_product_code]
                else:
                    # Strategy 2: Normalize the file code and try again
                    normalized_file_code = normalize_code_for_matching(file_product_code)
                    if normalized_file_code in normalized_webkodes:
                        matched_webkode = normalized_webkodes[normalized_file_code]
        ...
        for webkode in webkodes:
            ...
            if clean_webkode not in results['found']:
                ...
                if '-' in clean_webkode:
                    ...
                    if len(parts) >= 3:  # Format: LLDDDDD-DDDD-DD or DDDDD-DDDD-DD
                        # Normalize the missing webkode for comparison
                        normalized_missing_code = normalize_code_for_matching(clean_webkode)
                        base_product = '-'.join(normalized_missing_code.split('-')[:-1])  # e.g., "18486-0047"
                        ...
                        for media in media_files:
                            filename = media.get('filename', '')
                            if filename:
                                # Extract and normalize product code from filename
                                file_product_code = extract_product_code(filename)
                                normalized_file_code = normalize_code_for_matching(file_product_code)
                                
                                # Check if this file belongs to the same base product
                                if '-' in normalized_file_code:
                                    file_parts = normalized_file_code.split('-')
                                    if len(file_parts) >= 3:
                                        file_base = '-'.join(file_parts[:-1])
                                        
                                        # If same base product but different variant
                                        if file_base == base_product and normalized_file_code != normalized_missing_code:
                                            variant_suggestions.append({
                                                'url': media.get('image', ''),
                                                'filename': filename,
                                                'webkode': file_product_code,  # Use the original file code
                                                'original_webkode': clean_webkode,
                                                'suggestion_reason': f"Alternative variant ({file_product_code}) found for missing variant ({clean_webkode})"
                                            })
...
if __name__ == "__main__":
    main(), code)
            if match:
                # Return the numeric part (without letter prefix)
                return match.group(2)
            else:
                # Code doesn't start with letters, return as-is
                return code
