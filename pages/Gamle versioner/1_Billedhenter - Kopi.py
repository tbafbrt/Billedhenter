...
def create_suggested_filename(original_filename, searched_webkode, rename_alternatives, add_suggested_suffix):
    ...
    if rename_alternatives:
        ...
        if '-' in searched_webkode:
            parts = searched_webkode.split('-')
            if len(parts) >= 3:
                desired_variant = parts[-1]  # e.g., "50"
                
                # Parse the original filename to extract components
                if '_' in original_filename:
                    # Split filename: "IT22828-0029-00_01.jpg" -> ["IT22828-0029-00", "01.jpg"]
                    base_part = original_filename.split('_')[0]  # "IT22828-0029-00"
                    suffix_part = original_filename.split('_')[1]  # "01.jpg"
                    
                    # Replace the last variant part in base
                    if '-' in base_part:
                        base_parts = base_part.split('-')
                        if len(base_parts) >= 3:
                            # Replace "00" with "50": IT22828-0029-00 → IT22828-0029-50
                            base_parts[-1] = desired_variant
                            new_base = '-'.join(base_parts)  # "IT22828-0029-50"
                            
                            # Reconstruct filename: "IT22828-0029-50_01.jpg"
                            new_filename = f"{new_base}_{suffix_part}"
                        else:
                            # Fallback if format doesn't match expected pattern
                            new_filename = f"{searched_webkode}_{original_filename}"
                    else:
                        new_filename = f"{searched_webkode}_{original_filename}"
                else:
                    # Handle filenames without underscore
                    # Try to extract file extension
                    if '.' in original_filename:
                        name_part, ext_part = original_filename.rsplit('.', 1)
                        # Try to parse the base code from filename
                        if '-' in name_part:
                            name_parts = name_part.split('-')
                            if len(name_parts) >= 3:
                                name_parts[-1] = desired_variant
                                new_filename = f"{'-'.join(name_parts)}.{ext_part}"
                            else:
                                new_filename = f"{searched_webkode}.{ext_part}"
                        else:
                            new_filename = f"{searched_webkode}.{ext_part}"
                    else:
                        new_filename = f"{searched_webkode}_{original_filename}"
